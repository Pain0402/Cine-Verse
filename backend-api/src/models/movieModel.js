...
const MovieModel = {
  findAll: async (filters = {}) => {
    ...
    if (filters.release_date) {
      query = query.where("movies.release_date", filters.release_date);
    }
  ...
  create: async (movieData) => {
    // 1. Kiểm tra và thêm tmdb_id ngẫu nhiên nếu cần
    if (!movieData.tmdb_id) {
      // Tạo một số nguyên ngẫu nhiên 7 chữ số làm placeholder
      movieData.tmdb_id = Math.floor(Math.random() * 9000000) + 1000000;
    }

    // 2. Tách genres và các chi tiết khác của phim
    const { genres, ...movieDetails } = movieData;
    const [newMovie] = await db(TABLE_NAME).insert(movieDetails).returning("*");

    // 3. Xử lý genres nếu có
